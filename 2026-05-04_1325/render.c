...
void drawTriangleNormals(uint32_t* buffer, int bufferWidth, int bufferHeight,
                         float x1, float y1, float z1, uint32_t c1,
                         float x2, float y2, float z2, uint32_t c2,
                         float x3, float y3, float z3, uint32_t c3) {
    const float d = 1.0f; // Entfernung der Projektionsebene
    ...
    // Perspektiv-korrekte Interpolation vorbereiten
    float invZ1 = 1.0f / z1;
    float invZ2 = 1.0f / z2;
    float invZ3 = 1.0f / z3;

    // Normalen des Dreiecks berechnen
    ...
    if (length < 1e-6f) return; // Degeneriertes Dreieck
    nx /= length; ny /= length; nz /= length;

    // Baryzentrische Denominator
    float denom = (py2 - py3) * (px1 - px3) + (px3 - px2) * (py1 - py3);
    ...
    for (int y = minY; y <= maxY; ++y) {
        for (int x = minX; x <= maxX; ++x) {
            ...
            if (w1 >= 0.0f && w2 >= 0.0f && w3 >= 0.0f) {
                ...
                int idx = y * bufferWidth + x;
                if (zInterpolated >= zBuffer[idx]) continue;
                zBuffer[idx] = zInterpolated;

                // Normalen interpolieren (für großes Dreieck pro Pixel)
                float nx_pixel = nx;
                float ny_pixel = ny;
                float nz_pixel = nz;

                // Normale in RGB umwandeln [-1,1] → [0,255]
                uint32_t r = (uint32_t)((nx_pixel * 0.5f + 0.5f) * 255.0f);
                uint32_t g = (uint32_t)((ny_pixel * 0.5f + 0.5f) * 255.0f);
                uint32_t b = (uint32_t)((nz_pixel * 0.5f + 0.5f) * 255.0f);
                uint32_t a = 255;

                // Blau (Z) invertieren, da die Engine mit umgekehrter Z richtung arbeitet [0,255] → [255,0]
                b = 255 - b;
...
void drawTriangle_(uint32_t* buffer, int bufferWidth, int bufferHeight, int mode,
    ...
    } else if (mode == RENDER_MODE_NORMALS) {
        drawTriangleNormals(buffer,bufferWidth,bufferHeight,x1,y1,z1,c1,x2,y2,z2,c2,x3,y3,z3,c3);
