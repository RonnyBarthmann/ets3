...
int lua_free_object(lua_State* L) {
    int id = luaL_checkinteger(L,1);
    free_object(id);
    return 0;
}
...
int lua_get_camera(lua_State* L)
{
    ...
    lua_pushnumber(L, cameraYaw);
    lua_setfield(L, -2, "yaw");

    lua_pushnumber(L, cameraPitch);
    lua_setfield(L, -2, "pitch");

    lua_pushnumber(L, cameraRoll);
    lua_setfield(L, -2, "roll");

    lua_pushnumber(L, cameraX);
    lua_setfield(L, -2, "x");

    lua_pushnumber(L, cameraY);
    lua_setfield(L, -2, "y");

    lua_pushnumber(L, cameraZ);
    lua_setfield(L, -2, "z");
...
int lua_set_camera(lua_State* L)
{
    ...
    cameraYaw = (float)luaL_checknumber(L, -1);
    lua_pop(L, 1);

    lua_getfield(L, 1, "pitch");
    cameraPitch = (float)luaL_checknumber(L, -1);
    lua_pop(L, 1);

    lua_getfield(L, 1, "roll");
    cameraRoll = (float)luaL_checknumber(L, -1);
    lua_pop(L, 1);

    lua_getfield(L, 1, "x");
    cameraX = (float)luaL_checknumber(L, -1);
    lua_pop(L, 1);

    lua_getfield(L, 1, "y");
    cameraY = (float)luaL_checknumber(L, -1);
    lua_pop(L, 1);

    lua_getfield(L, 1, "z");
    cameraZ = (float)luaL_checknumber(L, -1);
    lua_pop(L, 1);
...
int lua_init(lua_State* L) {
    ...
    framebuffer = outputInit(WINDOW_WIDTH,WINDOW_HEIGHT,WINDOW_SCALE);
    inputInit();
    timerInit();
...
int lua_step(lua_State* L) {
        ...
        if (keyboard&KEY_RIGHT) {
            ...
            cameraMode = RENDER_MODE_NORMALS;
        }
        if (keyboard&KEY_UP) {
            cameraDistance += 0.1;
...
//        updateChunks();
        ...
        //if (delay > 0.1) {
        //    cameraDistance *= 0.9;
        //    printf("frame rate to low ... changed cameraDistance to %f\n",cameraDistance);
        //}
...
lua_State* InitLUA() {
    ...
    lua_pushcfunction(L, lua_free_object);
    lua_setfield(L,-2,"free_object");

    lua_pushcfunction(L, lua_shouldQuit);
    lua_setfield(L,-2,"shouldQuit");
    
    lua_pushcfunction(L, lua_get_camera);
    lua_setfield(L,-2,"get_camera");
    
    lua_pushcfunction(L, lua_set_camera);
